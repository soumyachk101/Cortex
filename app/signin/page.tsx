    ...
    <div className="min-h-screen flex items-center justify-center bg-alabaster p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-serif text-3xl font-semibold text-forest">Welcome Back</h1>
          <p className="text-text-secondary mt-2">Sign in to your Cortex account</p>
        </div>
        ...
        <div className="bg-white rounded-card shadow-botanical-lg border border-stone/50 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn-botanical w-full flex items-center justify-center gap-2 group mt-2">
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><span>Sign In</span><ArrowRight size={16} className="group-hover:translate-x-1 transition-transform duration-300" /></>}
            </button>
          </form>

          <p className="text-center text-sm text-text-secondary mt-6">
            Don&apos;t have an account?{' '}
            <Link href="/signup" className="text-sage font-medium hover:text-forest transition-colors duration-300">Sign Up</Link>
          </p>
        </div>
      </div>
    </div>
